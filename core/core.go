...
func Render(input []byte, output io.Writer) error {
	// Catpuccin Frappe, copied from https://github.com/alecthomas/chroma/blob/84d187e5ec9e201e8b11dd33095d45e7c3424089/styles/catppuccin-frappe.xml
	// But *without* background color
	myStyle := chroma.MustNewStyle(
		"catpuccin-frappe-without-bg",
		chroma.StyleEntries{
			chroma.Background:               "", // <- !
			chroma.CodeLine:                 "#c6d0f5",
	...
	err = tmpl.ExecuteTemplate(output, "preview.html", note)
	if err != nil {
